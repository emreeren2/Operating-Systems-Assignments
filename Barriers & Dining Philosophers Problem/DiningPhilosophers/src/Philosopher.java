...
public class Philosopher extends Thread {
	...
	// if current philosopher is hungary and
	// right and left philosophers of current philosopher, are not eating (means forks are available for current philosopher)
	// make current philosopher's state as eating
	...
	public void run() {
		...
		int walkTime = rand.nextInt(9000) + 1000;
		try{
			Philosopher.sleep(walkTime); // implementing walking of philosopher
		}
		catch (InterruptedException e){
			e.printStackTrace();
		}
		...
		for(int i=0; i<N; i++){ // barrier for philosophers to wait each other
			barriers[i].release();
		}
		for(int j=0; j<N; j++){
			try {
		...
		table.StartDining_GUI(id); // dining state
		int thinkTime = rand.nextInt(9000) + 1000;
		try{
			while(true) {
				sleep(thinkTime); // philosopher is thinking

				table.Hungry_GUI(id); // taking fork
				mutex.acquire();      // enter critical region
				state[id] = HUNGRY;   // record philosopher is hungry
				test(id);			  // try to acquire 2 forks
				mutex.release();      // exit critical region
				sem[id].acquire();    // block if forks were not acquired

				table.ForkTake_GUI(id); // eating
				table.Eating_GUI(id);

				table.StopEating_GUI(id); // putting forks
				mutex.acquire();		  // enter critical region
				state[id] = THINKING;     // philosopher finished eating
				table.ForkPut_GUI(id);    // visualizing that puts are available
				test((id+1)%N);        // checking, can left of current philosopher eat
				test((id+N-1)%N);      // checking, can right of current philosopher eat
				mutex.release(); 	     //  exit critical region
			}
		}
		catch (InterruptedException e){
